...
public class Main{
    ...
    public static void userRegister(){
        ...
        User user = new User(u_name,name,password);
    ...
    public static void userLogin(){
        ...
        System.out.print("Enter Your Password: ");
        String password = input.nextLine();
        ArrayList<User> user = adm.getUsers();
        User user_val = user.stream().filter(u -> u.getU_Name().equals(u_name)).findFirst().orElse(null);
        try {
            Thread.sleep(1000);
        }catch (Exception e){
            System.out.println(e.getMessage());
